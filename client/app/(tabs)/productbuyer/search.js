//client/app/(tabs)/productbuyer/search.js
...
export default function ProductBuyerSearchScreen() {
  const router = useRouter();
  const [query, setQuery] = useState('');
  const [searchSubmitted, setSearchSubmitted] = useState(false);
  const [recentSearches, setRecentSearches] = useState(['maize', 'milk', 'eggs']);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.0.100:5000/api/productbuyer/search?query=${encodeURIComponent(trimmedQuery)}`
      );
      ...
      {searchSubmitted && query ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    router.push({
                      pathname: '/productbuyer/listingdetails',
                ...
                >
                  ...
                  <View style={styles.resultDetails}>
                    ...
                    <Text style={styles.resultPrice}> {item.price_per_unit} </Text>
                    <Text style={styles.resultProvider}>{item.farmer_name}</Text>
                  </View>
                </TouchableOpacity>
              )}
        ...
        <View style={styles.recentContainer}>
          ...
          {recentSearches.length === 0 && (
            <Text style={styles.noRecent}>No suggestions</Text>
          )}
        </View>
      )}
    </View>
