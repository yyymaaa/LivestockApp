// client/app/(tabs)/productbuyer/profile.js
...
export default function Profile() {
  ...
  const productBuyerId = 4; // Temporary user_id for Product Buyer
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await fetch(`http://192.168.0.100:5000/api/productbuyer/profile/${productBuyerId}`);
        const data = await response.json();
        console.log('Product Buyer Profile fetched:', data);
        setUser(data.user);
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(`http://192.168.0.100:5000/api/productbuyer/profile/${productBuyerId}`, {
        method: 'PUT',
